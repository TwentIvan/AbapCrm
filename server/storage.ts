...
export interface IStorage {
  ...
  getBusinessScenariosBySourcePartner(sourcePartnerId: string): Promise<BusinessScenario[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getBusinessScenariosByOrganization(organizationId: string): Promise<BusinessScenario[]> {
    const scenarios = await db
      .select()
      .from(businessScenarios)
      .where(eq(businessScenarios.organizationId, organizationId))
      .orderBy(desc(businessScenarios.createdAt));
    return scenarios;
  }

  async getBusinessScenariosBySourcePartner(sourcePartnerId: string): Promise<BusinessScenario[]> {
    const scenarios = await db
      .select()
      .from(businessScenarios)
      .where(eq(businessScenarios.sourcePartnerId, sourcePartnerId))
      .orderBy(desc(businessScenarios.createdAt));
    return scenarios;
  }
  ...
  async getTasks(userId: string, organizationId: string): Promise<Task[]> {
    ...
    const result = await db.select({
      ...
      agentModelId: tasks.agentModelId,
      budgetCapEur: tasks.budgetCapEur,
      mcpConfigIds: tasks.mcpConfigIds,
      connectionWorkflowId: tasks.connectionWorkflowId,
  ...
  async getAllPlanningWindowsForUser(userId: string): Promise<(PlanningWindow & { project: Project | null })[]> {
    // Use leftJoin to include windows without projects (standalone planning windows)
    const result = await db
      .select()
